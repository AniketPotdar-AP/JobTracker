...
export async function exportApplicationsToXlsx(applications: Application[], owner: string) {
  ...
  XLSX.writeFile(book, `jobtrack-${safeOwner}-${new Date().toISOString().slice(0, 10)}.xlsx`);
}
