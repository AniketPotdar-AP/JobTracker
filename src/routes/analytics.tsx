...
export const Route = createFileRoute("/analytics")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Trends and conversion metrics for your job applications.",
      },
    ],
  }),
...
function AnalyticsPage() {
  ...
  const { monthly, bySource, statusData, funnel } = useMemo(() => {
    ...
    for (const a of apps) {
      ...
      if (
        ...
        a.statusHistory.some((h) => INTERVIEW_STATUSES.includes(h.status))
      ) {
        interviewed += 1;
      }
      if (
        a.status === "offer" ||
        a.status === "joined" ||
        a.statusHistory.some(
          (h) => h.status === "offer" || h.status === "joined",
        )
      ) {
        offered += 1;
      }
  ...
  const conversion = funnel.total
    ? Math.round((funnel.interviewed / funnel.total) * 100)
    : 0;
  const offerRate = funnel.total
    ? Math.round((funnel.offered / funnel.total) * 100)
    : 0;
  ...
  return (
    <>
      <PageHeader
        title="Analytics"
        description="Understand your job search performance"
      />
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <MetricCard
          ...
          value={`${conversion}%`}
          sub={`${funnel.interviewed} reached an interview`}
        />
        <MetricCard
          label="Offer rate"
          value={`${offerRate}%`}
          sub={`${funnel.offered} offers received`}
        />
      </div>
