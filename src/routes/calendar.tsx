...
export const Route = createFileRoute("/calendar")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Interviews, follow-ups, and deadlines at a glance.",
      },
    ],
  }),
...
function CalendarPage() {
  ...
  const entriesByDay = useMemo(() => {
    ...
    for (const a of apps) {
      ...
      if (a.interviewDate) {
        ...
        if (
          !hasAdded(dayStr, "interview") &&
          !hasAdded(dayStr, "recruiter_call")
        ) {
          markAdded(dayStr, "interview");
      ...
      if (a.rejectionDate) {
        ...
        const kind: EntryKind =
          a.status === "ghosted" ? "ghosted" : "rejection";
        if (!hasAdded(dayStr, kind)) {
          markAdded(dayStr, kind);
  ...
  const selectedEntries = selected
    ? (entriesByDay.get(format(selected, "yyyy-MM-dd")) ?? [])
    : [];
              ...
              {days.map((d) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={cn(
                        "text-[11px] sm:text-xs font-semibold shrink-0 text-center sm:text-left truncate w-full",
